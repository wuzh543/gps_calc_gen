...
void process_subframe(struct channel *x)
{
  ...
  uint8_t subframe_id = (data[5] & 0x1C) >> 2;  // extract subframe id  &0001 1100 再右移2
  // every subframe contains a TOW, 
  // at this point a psuedorange measurment can be taken
  x->nav.tow = ((data[3] << 9) | (data[4] << 1) | ((data[5] & 0x80) >> 7)) * 6;//data[5]&1000 0000 提取交接字前17位，每6s加一
...
void process_nav_bit(struct channel *x, char bit)
{
  ...
  x->nav.subframe[x->nav.subframe_write_pos++] = bit;//会出现subframe_write_pos=300？
  ...
  case SF_SEARCHING:
  {
	  ...
	  {
		  fastgps_printf("found preamble at bit %d of PRN %d\n", x->nav.subframe_write_pos, x->prn_num);
	  ...
	  if (x->nav.subframe_write_pos == SUBFRAME_LENGTH - 1)
	  {
		  ...
		  for (pc_idx = 0; pc_idx < MAX_PREAMBLE_CANDIDATES; pc_idx++)
			  x->nav.preamb_cor[pc_idx] = 0; //prepare for SF_VERIFYING
	  }
	  break;
  }
    case SF_VERIFYING:
    {
      ...
      for (pc_idx = 0; pc_idx < x->nav.num_preamb_cand; pc_idx++)
      {
        ...
        if (x->nav.subframe_write_pos == x->nav.preamb_cand[pc_idx])  //subframe_write_pos=0
        {
          // test it to see if the preamble appears again at this offset开端, and that the parity bits check out OK
          for (sf_idx = 0; sf_idx < PREAMBLE_LENGTH; sf_idx++)
          ...
          {
            char par_buf[32];//30bit+前一字的最后2bit
            short par_idx;
            char parity;

            // now, build a continuous buffer to check the parity奇偶校验 bits
            ...
            {
              unsigned tempi = (x->nav.subframe_write_pos - PREAMBLE_LENGTH) % SUBFRAME_LENGTH;//保证在0-299之间
              ...
              for (i = 0; i < SUBFRAME_LENGTH; i++)
                    x->nav.subframe[i] = subframe_temp[i];//调整好的一帧

              x->nav.parity = parity;
              x->nav.subframe_write_pos = PREAMBLE_LENGTH;//寻找成功后  subframe_write_pos=8
        ...
        for (word_idx = 1; word_idx < 10; word_idx++)
        {
          ...
          for (unsigned pl_idx = 0; pl_idx < 24; pl_idx++)
          {
            ...
            if(temp_parity == -1) // invert the bits in this word if needed
              payload_bits[pl_idx] = payload_bits[pl_idx] ^ 1;  // XOR异或
          }
